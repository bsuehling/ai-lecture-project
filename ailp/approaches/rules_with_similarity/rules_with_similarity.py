...
class RulesWithSimilarityModel(PredictionModel):
    def __init__(
        self, train_path: str | PathLike = None, test_path: str | PathLike = None
    ) -> None:
    ...
    def predict_graph(self, parts: set[Part]) -> Graph:
        ...
        def degree(part):
            return degree_dict[part.part_id][2] if part.part_id in degree_dict else 1
        ...
        def rate_edge(edge: tuple[Part, Part]):
            ...
            src_i, tgt_i = self._part_ids.index(src_part.part_id), self._part_ids.index(
                tgt_part.part_id
            )
    ...
    def train(self, train_graphs: list[Graph], eval_graphs: list[Graph]):
        ...
        family_num_parts_lookup = self.calc_num_parts_per_family_lookup(
            family_ids, train_graphs
        )
        part_to_family_lookup = self.calc_family_for_part_lookup(
            part_ids, family_ids, train_graphs
        )

        # Calculate the expected degrees for each part
        pid_degree_dict = self.calc_degree_lookup(
            train_graphs, part_ids, extract=extract_part_id
        )
        fid_degree_dict = self.calc_degree_lookup(
            train_graphs, family_ids, extract=extract_family_id
        )
        ...
        for pid, pid_degrees in pid_degree_dict.items():
            ...
            degree_dict[pid] = (
                (pid_degrees[0] + fid_degrees[0] / 2),
                (pid_degrees[1] + fid_degrees[1] / 2),
                (pid_degrees[2] + fid_degrees[2] / 2),
            )

        # Create matrices for id x id -> likelihood of id x id
        pid_edge_likelihood = self.calc_edge_likelihood_lookup(
            train_graphs, part_ids, extract=extract_part_id
        )
        fid_edge_likelihood = self.calc_edge_likelihood_lookup(
            train_graphs, family_ids, extract=extract_family_id
        )
        ...
        model = {"edge_score_matrix": edge_score_matrix, "degree_dict": degree_dict}
        save_model(model, self._train_path)
        print("Training completed")

    # Calculates a matrix for each possible edge between
    def calc_edge_likelihood_lookup(
        self, train_graphs: list[Graph], all_ids: list[str], extract=None
    ):
        ...
        for graph in train_graphs:
            for src, neighbors in graph.edges.items():
                ...
                for tgt in neighbors:
                    ...
                    edge_count_matrix[src_i, tgt_i] = (
                        edge_count_matrix[src_i, tgt_i] + 1
                    )
        ...
        for graph in train_graphs:
            for src in graph.nodes:
                for tgt in graph.nodes:
                    ...
                    src_i = all_ids.index(extract(src.part))
                    tgt_i = all_ids.index(extract(tgt.part))
                    possibility_count_matrix[src_i, tgt_i] = (
                        possibility_count_matrix[src_i, tgt_i] + 1
                    )

        element_wise_division = np.vectorize(lambda x, y: x / y if y > 0 else 0)
        edge_likelihood = element_wise_division(
            edge_count_matrix, possibility_count_matrix
        )
    ...
    def calc_num_parts_per_family_lookup(
        self, family_ids: list[str], train_graphs: list[Graph]
    ):
        ...
        for graph in train_graphs:
            for node in graph.nodes:
                ...
                if fid not in data_dict:
                    data_dict[fid] = []
                if pid not in data_dict[fid]:
                    data_dict[fid].append(pid)
        ...
        print(
            f"Calculated num parts for family lookup (values between {min(matrix)} and {max(matrix)})"
        )
        return matrix

    def calc_family_for_part_lookup(
        self, part_ids: list[str], family_ids: list[str], train_graphs: list[Graph]
    ):
        ...
        print(
            f"Calculated part-id-index -> family-id-index lookup with {np.sum(matrix == -1)} misses"
        )
        return matrix
    ...
    def create_similarity_matrix(self, all_ids: list[str], wtv: Word2Vec):
        ...
        for src_i in range(len(all_ids)):
            for tgt_i in range(len(all_ids)):
                ...
                if not (src_id in wtv.wv and tgt_id in wtv.wv):
                    misses += 1
                    continue
                matrix[src_i][tgt_i] = abs(wtv.wv.similarity(src_id, tgt_id))

        print(
            f"Produced ({size}x{size}) similarity matrix with {(misses/max_misses *100)}% misses"
        )
        return matrix

    def create_edge_score_matrix(
        self,
        part_ids,
        family_ids,
        part_to_family,
        ...
        for src_pi in range(len(part_ids)):
            ...
            for tgt_pi in range(len(part_ids)):
                ...
                for tgt_replacement_pi in range(len(part_ids)):
                    ...
                    families_are_known = (
                        src_fi != -1 and tgt_fi != -1 and tgt_replacement_fi != -1
                    )
        ...
        print(
            f"Created edge score matrix {len(matrix)}x{len(matrix[0])} in range [{np.min(matrix)}, {np.max(matrix)}]"
        )
        return matrix
