...
class NodeToVecModel(PredictionModel):
    ...
    def predict_graph(self, parts: set[Part]) -> Graph:
        ...
        def lookup_part_id(id, fallback=None): return part_id_lookup[id] if id in part_id_lookup else fallback
        def lookup_family_id(id, fallback=None): return family_id_lookup[id] if id in family_id_lookup else fallback

        def degree(part): return degree_lookup[part.part_id][2] if part.part_id in degree_lookup else 1
        def max_degree(part): return degree_lookup[part.part_id][1] if part.part_id in degree_lookup else 1
        ...
        def select_source_part():
            ...
            if max_num_open_connectors > 0:
                return next(filter(lambda p: open_connections(p) == max_num_open_connectors, used_parts), None)
            # Otherwise go off the maximum number of degrees
            max_num_open_connectors = max([max_open_connections(p) for p in used_parts])
            return next(filter(lambda p: max_open_connections(p) == max_num_open_connectors, used_parts), None)
            ...
            if wanted_neighbors is not None:
                ...
                for tgt in wanted_neighbors:
                    if tgt in [p.part_id for p in spare_parts]:
                        return next(filter(lambda p: p.part_id == tgt, spare_parts), None)
            ...
            if wanted_neighbors is not None:
                ...
                for tgt in wanted_neighbors:
                    if tgt in [p.family_id for p in spare_parts]:
                        return next(filter(lambda p: p.family_id == tgt, spare_parts), None)
        ...
        while spare_parts:
            ...
            self.log(1, f"Selected {src_part.part_id} with {open_connections(src_part)} open slots")
    ...
    def train(self, train_graphs: list[Graph], eval_graphs: list[Graph]):
        ...
        wtv_part_ids   = self.create_word_2_vec_model(train_graphs, extract=extract_part_id)
        wtv_family_ids = self.create_word_2_vec_model(train_graphs, extract=extract_family_id)

        part_ids, family_ids = self.read_ids()
        
        predictor_part_ids   = self.create_predictor_model(train_graphs, part_ids,  wtv_part_ids,   extract=extract_part_id)
        predictor_family_ids = self.create_predictor_model(train_graphs, family_ids, wtv_family_ids, extract=extract_family_id)

        part_id_lookup   = self.calc_lookup_table(part_ids, wtv_part_ids, predictor_part_ids)
        family_id_lookup = self.calc_lookup_table(family_ids, wtv_family_ids, predictor_family_ids)
        degree_lookup    = self.calc_degree_lookup(train_graphs)

        model = { "part_id_lookup": part_id_lookup, "family_id_lookup": family_id_lookup, "degree_lookup": degree_lookup }
    ...
    def create_predictor_model(self, train_graphs: list[Graph], all_ids: list[str],
                               word_to_vec,
                               extract=lambda part: part.part_id,
                                ):
        examples = self.calc_training_data(train_graphs, all_ids, extract=extract)

        def embedded(id): return word_to_vec.wv[id] if id in word_to_vec.wv else None
    ...
    def calc_training_data(self, train_graphs: list[Graph], all_ids: list[str],
                           extract=lambda part: part.part_id,
                           ):
    ...
    def calc_degree_lookup(self, train_graphs: list[Graph]) -> dict[str, tuple[int, int, int]]:
        degree_dict = {}
    ...
    def calc_lookup_table(self, all_ids, word_to_vec, predictor):
        ...
        for id in all_ids:
            ...
            if ordered is None: misses += 1
            table[id] = ordered

        print(f"Calculated lookup table for {len(all_ids)} Ids. Misses: {misses} ({(misses / len(all_ids)) * 100}%)")
        return table
